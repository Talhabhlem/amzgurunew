...
@section('title', trans_choice('l5cp-user::default.permission', 2))
@section('page-header', trans_choice('l5cp-user::default.permission', 2))
...
@section('content')
  ...
  <table class="table table-bordered table-striped">
      ...
      @foreach($users as $user)
        <tr>
          <td>
            <a href="{{ action('\Askedio\Laravelcp\User\Http\Controllers\PermsController@edit', ['id' => $user->id]) }}" class="btn btn-default"><i class="fa fa-fw fa-pencil"></i></a>
            <a href="{{ action('\Askedio\Laravelcp\User\Http\Controllers\PermsController@destroy', ['id' => $user->id]) }}" class="btn btn-danger delete" data-type="DELETE"><i class="fa fa-fw fa-trash"></i></a>
          </td>
          <td>{{ $user->name }}</td>
...
@section('page-header-right')
    <a href="{{ action('\Askedio\Laravelcp\User\Http\Controllers\PermsController@create') }}" class=" pull-right btn btn-primary btn-xl">Create</a>
@endsection