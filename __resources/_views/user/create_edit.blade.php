  ...
  @if (isset($user))
	...
	@endif
      ...
      <div class="pull-left">
        ...
        <a href="{{ action('app\Http\Controllers\HomeController@index') }}" class="btn-responsive btn btn-danger">{{ trans('lcp::button.cancel') }}</a>
      </div>

      @if (isset($user))
      <div class="pull-right">
        <button type="reset" class="btn-responsive btn btn-default">{{ trans('lcp::button.reset') }}</button>
        <a data-row="[{ $user->id }}"  href="{{ action('app\Http\Controllers\HomeController@destroy', ['id' => $user->id]) }}" class="delete btn-responsive btn btn-warning">{{ trans('lcp::button.delete') }}</a>
      </div>
      @endif
